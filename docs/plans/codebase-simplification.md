...
Codex との協調分析により特定された、自分が使う上での失敗シナリオと対策。

### F1: tmux/非tmux の特定経路が死ぬ

| 項目 | 内容 |
|------|------|
| **発生条件** | 共通化で tmux 専用ロジックが非tmux モードに混入、または逆 |
| **影響** | 特定環境で動かなくなる |
| **検出タイミング** | リファクタリング後の使用時 |

**対策**:
- [ ] 各変更後に tmux/非tmux 両方でテスト
- [ ] 最低限のスモークテストを手動で実行

### F2: 削除順序ミスでコマンドが起動不能に

| 項目 | 内容 |
|------|------|
| **発生条件** | インラインフォールバック削除 → ヘルパー削除 で「どちらもなくなる」瞬間が発生 |
| **影響** | コマンドが動かなくなる |
| **検出タイミング** | 実装中 |
...
- [ ] 削除は最後に回す
- [ ] 削除前に `rg` で再確認
- [ ] git history から復元可能なので致命的ではない

## 対策の優先度

| 優先度 | 対策 |
|--------|------|
| 🔴 高 | 各変更後に tmux/非tmux 両方でテスト |
| 🔴 高 | 1コミット = 1機能完結 |
| 🟠 中 | 削除は最後に回す |
| 🟡 低 | スモークテストの自動化（必要に応じて） |
